                    ...
                    const Mat &descriptors_l_, const Mat &descriptors_r_, ORBextractor::Ptr ORBextractorLeft, ORBextractor::Ptr ORBextractorRight)
{
    ...
    const float minZ = Parameters::base_fx_ / Parameters::fx_;
    const float minD = 0;
    const float maxD = Parameters::fx_;
...
{
    mfGridElementWidthInv = static_cast<float>(FRAME_GRID_COLS) / (Parameters::max_X_ - Parameters::min_X_);
    mfGridElementHeightInv = static_cast<float>(FRAME_GRID_ROWS) / (Parameters::max_Y_ - Parameters::min_Y_);
    posX = round((kp.pt.x - Parameters::min_X_) * mfGridElementWidthInv);
    posY = round((kp.pt.y - Parameters::min_Y_) * mfGridElementHeightInv);
...
vector<size_t> ORBextractor::GetFeaturesInArea(const ORBextractor::Ptr ORBextractorLeft, const vector<cv::KeyPoint> &keypoints_l_, const float &x, const float &y, const float &r, const int minLevel, const int maxLevel)
{
    ...
    int N = keypoints_l_.size();
    vIndices.reserve(N);
    const int nMinCellX = max(0, (int)floor((x - Parameters::min_X_ - r) * mfGridElementWidthInv));
    if (nMinCellX >= FRAME_GRID_COLS)
        return vIndices;
    const int nMaxCellX = min((int)FRAME_GRID_COLS - 1, (int)ceil((x - Parameters::min_X_ + r) * mfGridElementWidthInv));
    if (nMaxCellX < 0)
        return vIndices;

    const int nMinCellY = max(0, (int)floor((y - Parameters::min_Y_ - r) * mfGridElementHeightInv));
    if (nMinCellY >= FRAME_GRID_ROWS)
        return vIndices;

    const int nMaxCellY = min((int)FRAME_GRID_ROWS - 1, (int)ceil((y - Parameters::min_Y_ + r) * mfGridElementHeightInv));
    if (nMaxCellY < 0)
        return vIndices;
